...
pro spectral_fits_jmp_v2_90

  common counts, y, dy
  common weighting, dt, energy, Kwfe, Kwfp

  startSystime = systime() ;for debugging purposes, when did this program start
  date = ['2016','10','25'] ;year, month, day
  ;date = ['2017','09','25'] ;year, month, day
  ;date = ['2003','10','20'] ;year, month, day
  ; date = ['2004','01','01'] ;end date for program ;n15,16,18,19,m02
  ;date = ['2009','05','22'] ;year, month, day
...
;stop
  ...
  while fdoydate le enddoydate do begin
    ...
    restore, 'D:\POES_Data\MPE_Software\90_Degree_Detector\Pcounts_temp.sav'
...
;        stop
   ...
   ; stop
    ...
    for i = Nstart, Nend-1 do begin ;time loop ;EDP
      ...
      ;for i = 0, 100 do begin ;time loop ;EDP
    ...
    ;------------------------------------------------------------------------------------
count_rate_issue = 'yes'
if count_rate_issue eq 'yes' then begin
 ...
 ;             ;print, new_electroncount[*, index]
  ...
  ;  counter = 0
   ...
   ;       counter = counter + 1
    ...
    ;------------------------------------------------------------------------------------
    print, 'Noise Floor Turned Off'
    ;------------------------------------------------------------------------------------
    ;print, N_ELEMENTS(where(new_electronCount[0, *] eq 0.))
    ;NOISE_FLOOR_INDEX = where(new_electronCount[0, *] lt 2.5)
...
;satName is a string the the full name of the satellite being analyzed, so 'METOP02' is metop2.
      ...
      ;  	               rTime[i] = ((milli/1000.+sc)/60.+mn)/60.+hr
			...
			wpATTR3 = 'Multiply respective elements to ln of spectra, sum products together, and exp to get combined spectrum.'
                        ...
                        ;ofile = NCDF_CREATE('Z:\MEE_Data\Processed_MEPED_Data\'+satName+'\'+ofilename,/CLOBBER)
                        ;ofile = NCDF_CREATE('Z:\MEE_Data\Allison_Data\'+ofilename,/CLOBBER)
                        ;ofile = NCDF_CREATE('Z:\MEE_Data\Processed_MEPED_Data\MPE_v3\'+satName+'\'+ofilename,/CLOBBER)
                        ...
                        ;rtimeID = NCDF_VARDEF(ofile,'time',timedimID,/DOUBLE)
                       ...
                       NCDF_ATTPUT,ofile,BLC_FootID,'Units',BLC_FootID,/CHAR
                        ...
                        NCDF_ATTPUT,ofile,'Date_Created','APRIL 2014',/CHAR,/GLOBAL
...
pro ammendNCfiles_BLC_90,indSat
  ...
  while fdoydate le enddoydate do begin ;loop over days
    ...
    ;ncfile = 'Z:\MEE_Data\Processed_MEPED_Data\New_Processed\'+indSat+'\POES_combinedSpectrum_'+indSat+'_00_'+date[0]+date[1]+date[2]+'.nc'
    ncfile = 'Z:\MEE_Data\Processed_MEPED_Data\MPE_v3\'+fullSatName+'\POES_combinedSpectrum_'+indSat+'_90_'+date[0]+date[1]+date[2]+'.nc'
    ...
    if exists then begin
      ...
      if blc_exists then begin
        ...
        BLC_angle = (BLC[0:-2:2]+BLC[1:-1:2])/2. ;average in 16 sec instead of indiv 8 seconds. This is fancy code!
                          ...
                          BLCID = NCDF_VARDEF(ncid,'BLC_Angle',timedimID,/DOUBLE)
                          NCDF_ATTPUT,ncid,BLCID,'Full_Name',BLCATTR1,/CHAR
                          NCDF_ATTPUT,ncid,BLCID,'Units',BLCATTR2,/CHAR
                        ;  NCDF_CONTROL,ncid,/ENDEFEOCOUNTS_CORRECTEDID
                          NCDF_VARPUT,ncid,BLCID,BLC_angle
                          NCDF_CLOSE,ncid
  ;      stop
...
pro ammendNCfiles_magField_90,indSat
  ;this code is to update the NC file output to include pitch angles, which are kind of important.
  ;angle = 0 ;look at 0 degree telescopes
  sat = 'POES' ;only satellite this program works with, so don't change it.
  angle = 90
  ...
  while fdoydate le enddoydate do begin ;loop over days
    ...
    ;check validity of date
    ;ncfile = '/export/home/pecked/MEPED/data/'+'POES_combinedSpectrum_'+indSat+'_00_'+date[0]+date[1]+date[2]+'.nc'
    ;ncfile = 'Z:\MEE_Data\Processed_MEPED_Data\New_Processed\'+indSat+'\POES_combinedSpectrum_'+indSat+'_90_'+date[0]+date[1]+date[2]+'.nc'
    ncfile = 'Z:\MEE_Data\Processed_MEPED_Data\MPE_v3\'+fullSatName+'\POES_combinedSpectrum_'+indSat+'_90_'+date[0]+date[1]+date[2]+'.nc'
    ;ncfile = 'Z:\MEE_Data\Allison_Data\POES_combinedSpectrum_v2_'+indSat+'_00_'+date[0]+date[1]+date[2]+'.nc'
    exists = FILE_TEST(ncfile)
    if exists then begin
...
function pCounts,degree,type,sat,date
  ...
  ;  return,0
  ;endif
  cdf_exists = FILE_TEST('Z:\MEE_Data\POES_Data\'+filename)
  if cdf_exists then begin
  cdfid = CDF_OPEN('Z:\MEE_Data\POES_Data\'+filename,/READONLY)
...
function pCounts2,degree,type,sat,date
  ...
  if cdf_exists then begin
    ...
    ; Read data
    ; ---------------------------------------------------------------------
    varid = ncdf_varid(cdfid, 'mep_ele_tel90_cps_e1') ;electron telescope channel 1
    ncdf_varget, cdfid, varid, mep_ele_tel90_cps_e1
    varid = ncdf_varid(cdfid, 'mep_ele_tel90_cps_e2') ;electron telescope channel 2
    ncdf_varget, cdfid, varid, mep_ele_tel90_cps_e2
    varid = ncdf_varid(cdfid, 'mep_ele_tel90_cps_e3') ;electron telescope channel 3
    ncdf_varget, cdfid, varid, mep_ele_tel90_cps_e3
    varid = ncdf_varid(cdfid, 'mep_pro_tel90_cps_p1') ;proton telescope channel 1
    ncdf_varget, cdfid, varid, mep_pro_tel90_cps_p1
    varid = ncdf_varid(cdfid, 'mep_pro_tel90_cps_p2') ;proton telescope channel 2
    ncdf_varget, cdfid, varid, mep_pro_tel90_cps_p2
    varid = ncdf_varid(cdfid, 'mep_pro_tel90_cps_p3') ;proton telescope channel 3
    ncdf_varget, cdfid, varid, mep_pro_tel90_cps_p3
    varid = ncdf_varid(cdfid, 'mep_pro_tel90_cps_p4') ;proton telescope channel 4
    ncdf_varget, cdfid, varid, mep_pro_tel90_cps_p4
    varid = ncdf_varid(cdfid, 'mep_pro_tel90_cps_p5') ;proton telescope channel 5
    ncdf_varget, cdfid, varid, mep_pro_tel90_cps_p5
    varid = ncdf_varid(cdfid, 'mep_pro_tel90_cps_p6') ;proton telescope channel 6
    ncdf_varget, cdfid, varid, mep_pro_tel90_cps_p6
    ...
    varid = ncdf_varid(cdfid2, 'meped_alpha_90_sat')                ; pitch angle of the satellite
    ncdf_varget, cdfid2, varid, meped_alpha_90_sat
    ...
    for i = 0, end_time-1 do begin
      ...
      if i eq 0 then index1 = 0 ;3
      if i eq 0 then index2 = 7 ;10
      if i eq 0 then index3 = 4
   ...
   ;   if i eq 1 then index3 = 23
      ...
      Channel_1_Temp = mep_ele_tel90_cps_e1[index1:index2]
      Channel_2_Temp = mep_ele_tel90_cps_e2[index1:index2]
      Channel_3_Temp = mep_ele_tel90_cps_e3[index1:index2]
      PChannel_1_Temp = mep_pro_tel90_cps_p1[index1:index2]
      PChannel_2_Temp = mep_pro_tel90_cps_p2[index1:index2]
      PChannel_3_Temp = mep_pro_tel90_cps_p3[index1:index2]
      PChannel_4_Temp = mep_pro_tel90_cps_p4[index1:index2]
      PChannel_5_Temp = mep_pro_tel90_cps_p5[index1:index2]
      PChannel_6_Temp = mep_pro_tel90_cps_p6[index1:index2]
      ...
      pitch_angle[i]    =  meped_alpha_90_sat[index3]
...
;     stop
    save, BLC_Angle, Bfofl, Blocal, pparticleCount, particleCount, msec_time, geogLL, time, MLT, foflLL, pitch, lValue, filename='D:\POES_Data\MPE_Software\90_Degree_Detector\Pcounts_temp.sav'
